...
#设置输出目录
output_dir = 'borderline-smote'
if not os.path.exists(output_dir):
    os.makedirs(output_dir)

for iteration in range(1):
    single = open(os.path.join(output_dir,f'{classifier} bor_smote {iteration} .csv'), 'a',
                  newline='')
    ...
    for inputfile in os.listdir("datasets\\relink dataset_csv"):
        ...
        for i in range(10):
            for train, test in skf.split(x, y):
                ...
                bor_smote_train_x, bor_smote_train_y = borderlineSMOTE.fit_resample(train_x, train_y)

                total_ratio = total_ratio + (len(bor_smote_train_y[bor_smote_train_y == 1]) / len(bor_smote_train_y))
                clf = classifier_for_selection[classifier]
                clf.fit(bor_smote_train_x, bor_smote_train_y)
